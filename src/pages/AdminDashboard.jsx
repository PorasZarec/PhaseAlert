...
import { supabase } from "../services/supabaseClient"; // IMPACT: Import Supabase
import { useAuth } from "../contexts/AuthContext";
import { toast } from "sonner"; // IMPACT: Import Toast trigger
...
const AdminDashboard = () => {
  ...
  const handleDeleteUser = async (userId) => {
    ...
    // In Supabase, deleting from Auth is strictly separate from deleting from Tables.
    // For now, let's delete the profile row.
...
const NavButton = ({ active, onClick, icon, label }) => (
  ...
  <a onClick={onClick} href="#"
    ...
    ${active ? "bg-blue-600 shadow-lg shadow-blue-500/30 text-white" : "border border-gray-400 text-gray-400 hover:bg-blue-600/90 hover:text-white"}`}
  >
    {icon}
    {/* Tooltip */}
    <span className="absolute left-full ml-5 px-3 py-1.5 bg-blue-600/90 rounded-md text-sm font-medium text-white whitespace-nowrap invisible opacity-0 scale-95 group-hover:visible group-hover:opacity-100 group-hover:scale-100 transition-all z-50">
      {label}
    </span>
  </a>
