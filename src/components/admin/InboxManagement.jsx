...
const InboxManagement = () => {
  ...
  // 2. Fetch Resident List (Profiles)
  ...
  // 3. Use your custom Hook for Messages
  // If activeTab is 'community', otherUserId is null.
  // If activeTab is 'direct', otherUserId is the selected resident's ID.
  const { messages, isLoading: isLoadingMessages, sendMessage, isSending } = useMessages(
    activeTab, 
    currentUser?.id, 
    selectedResident?.id
  );
            ...
            filteredResidents.map(resident => (
              ...
              >
                ...
                <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden shrink-0">
                  {resident.avatar_url ? (
                    <img src={resident.avatar_url} alt="" className="w-full h-full object-cover"/>
                  ) : (
                    <span className="text-gray-600 font-bold text-sm">
                        {resident.full_name ? resident.full_name[0].toUpperCase() : 'U'}
                    </span>
                  )}
                </div>
          ...
          <div className="flex items-center gap-2 lg:gap-3 flex-1 lg:flex-initial">
            {activeTab === 'community' ? (
              <>
                ...
                <div>
                  <h3 className="font-bold text-gray-800 text-sm lg:text-base">Community Chat</h3>
                  <p className="text-[10px] lg:text-xs text-gray-500">Public visibility</p>
                </div>
              </>
            ) : selectedResident ? (
              <>
              ...
              <div className="text-gray-400 text-sm lg:text-base">Select a resident to chat</div>
            )}
          </div>
        </div>

        {/* Messages List */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 lg:p-6 space-y-2 bg-gray-50/30">
          {isLoadingMessages ? (
        ...
        <div className="p-3 lg:p-4 border-t border-gray-100 bg-white shrink-0">
          <div className="flex gap-2 lg:gap-3 relative">
            <input
              ...
              placeholder={activeTab === 'direct' && !selectedResident ? "Select a contact first" : "Type your message..."}
